    ...
    def draw(frame,i):
        if i is not None:
            ...
            if c is not None:
                ...
                cv2.putText(frame,str(c),(c[0]+10,c[1]+10),cv2.FONT_HERSHEY_COMPLEX, 0.5, (255,0, 0), 2)
        return frame,c
            ...
            while vid.isOpened():
                ...
                if(f==0):
                    while 1:
                        ...
                        cv2.putText(frame,"ДОБРО ПОЖАЛОВАТЬ В MUSICMENT",(120,360),cv2.FONT_HERSHEY_COMPLEX, 1.5, (0,0, 255), 5)
                        cv2.putText(frame,"Нажмите 'Enter для продолжения ",(300,410),cv2.FONT_HERSHEY_COMPLEX, 1, (0,255, 0), 2)
                        cv2.putText(frame,"ИНСТРУКЦИИ ",(450,500),cv2.FONT_HERSHEY_COMPLEX, 1, (255,255,0), 2)
                        cv2.putText(frame,"Сначала, Покажите Руку, которую вы хотите использовать для Октав ",(80,540),cv2.FONT_HERSHEY_COMPLEX, 0.75, (255,255,0), 2)
                        cv2.putText(frame,"Затем Покажите Руку, которую Вы хотите использовать для Нот ",(100,570),cv2.FONT_HERSHEY_COMPLEX, 0.75, (255,255,0), 2)
                        cv2.putText(frame,"(Примечание: если вы будете использовать одну руку, то она будет использоваться только для Нот)",(100,610),cv2.FONT_HERSHEY_COMPLEX, 0.5, (0,0,255), 2)
                        # cv2.putText(frame,"Сначала, Покажите Руку, которую вы хотите использовать для Октав",(250,480),cv2.FONT_HERSHEY_COMPLEX, 0.75, (255,255,0), 2)
                        cv2.putText(frame,"Нажмите q/Q, чтобы Выйти",(10,50),cv2.FONT_HERSHEY_COMPLEX, 0.75, (255,0, 0), 2)
                        cv2.imshow('Musicment-Музыкальный инструмент', frame)
                        cv2.resizeWindow('Musicment-Музыкальный инструмент', 1080,720)
                        key1=cv2.waitKey(1)
                        if key1 & 0xFF == 13:
                            break
	...
	# desired button of your choice
                ...
                # Calculating the centroid of 6 points finger's tips and wrist point
                    ...
                    if centroid is not None:
                        ...
                        cv2.putText(frame,str(centroid),(centroid[0]+10,centroid[1]+10),cv2.FONT_HERSHEY_COMPLEX, 0.5, (255,0, 0), 2)
                    ...
                    if wri is not None and centroid is not None and all(a):
                        ...
                        cv2.putText(frame,str(wri),(wri[0]+10,wri[1]+10),cv2.FONT_HERSHEY_COMPLEX, 0.5, (255,0, 0), 2)
                        cv2.putText(frame,"Notes",(wri[0],wri[1]+50),cv2.FONT_HERSHEY_COMPLEX, 0.5, (0,0, 255), 2)
                    ...
                    if all(a) and centroid is not None and wri is not None:
                        ...
                        # Calculating the centroid of 5 finger's tips
                            ...
                            if centroid1 is not None:
                                ...
                                cv2.putText(frame,str(centroid1),(centroid1[0]+10,centroid1[1]+10),cv2.FONT_HERSHEY_COMPLEX, 0.5, (255,0, 0), 2)
                            ...
                            if wri1 is not None and centroid1 is not None and all(a1):
                                ...
                                cv2.putText(frame,str(wri1),(wri1[0]+10,wri1[1]+10),cv2.FONT_HERSHEY_COMPLEX, 0.5, (255,0, 0), 2)
                                cv2.putText(frame,"Октава",(wri1[0],wri1[1]+50),cv2.FONT_HERSHEY_COMPLEX, 0.5, (0,0, 255), 2)
                            ...
                            if all(a1) is not None and centroid1 is not None and wri1 is not None:
                                ...
                                #prints distance ratio for finger tips(for debug purpose)
                                print("Октавы"+str(sat1))
                                if not(sat[1]<0.4 and sat[2]<0.4 and sat[0]<0.4 and sat[4]>0.45):
                                    if (sat1[0]>0.5 and not sat1[1]>0.45 and not sat1[2]>0.45):
                                        cv2.putText(frame,"Октава: 4",(40,170),cv2.FONT_HERSHEY_COMPLEX, 0.5, (255,0, 0), 2)
                                        octave=math.pow(2,(4-1)-3)
                                    elif(sat1[0]>0.4 and sat1[1]>0.45 and not sat1[2]>0.45):
                                        cv2.putText(frame,"Октава: 5",(40,170),cv2.FONT_HERSHEY_COMPLEX, 0.5, (0,255, 0), 2)
                                        octave=math.pow(2,(5-1)-3)
                                    elif(sat1[1]>0.4 and sat1[2]>0.4 and sat1[0]>0.4):
                                        cv2.putText(frame,"Октава: 6",(40,170),cv2.FONT_HERSHEY_COMPLEX, 0.5, (0,0, 255), 2)
                                        octave=math.pow(2,(6-1)-3)
                                    else:
                                        cv2.putText(frame,"Октава: 3",(40,170),cv2.FONT_HERSHEY_COMPLEX, 0.5, (0,255, 255), 2)
                                        octave=math.pow(2,(3-1)-3)
                                else:
                                    if (sat1[0]>0.5 and not sat1[1]>0.45 and not sat1[2]>0.45):
                                        cv2.putText(frame,"Октава: 5",(40,170),cv2.FONT_HERSHEY_COMPLEX, 0.5, (255,0, 0), 2)
                                        octave=math.pow(2,(4)-3)
                                    elif(sat1[0]>0.4 and sat1[1]>0.45 and not sat1[2]>0.45):
                                        cv2.putText(frame,"Октава: 6",(40,170),cv2.FONT_HERSHEY_COMPLEX, 0.5, (0,255, 0), 2)
                                        octave=math.pow(2,(5)-3)
                                    elif(sat1[1]>0.4 and sat1[2]>0.4 and sat1[0]>0.4):
                                        cv2.putText(frame,"Октава: 7",(40,170),cv2.FONT_HERSHEY_COMPLEX, 0.5, (0,0, 255), 2)
                                        octave=math.pow(2,(6)-3)
                                    else:
                                        cv2.putText(frame,"Октава: 4",(40,170),cv2.FONT_HERSHEY_COMPLEX, 0.5, (0,255, 255), 2)
                    ...
                    # hand 2 ----end
                        ...
                        if len(results.multi_hand_landmarks)==1 or len(results.multi_hand_landmarks)==2:
                            ...
                            if (sat[0]>0.5 and not sat[1]>0.5 and not sat[2]>0.5):
                                cv2.putText(frame,"Нота: Re/D",(40,130),cv2.FONT_HERSHEY_COMPLEX, 0.5, (255,0, 0), 2)
                                cv2.putText(frame,"Частота: "+str(sa[1]*octave),(40,150),cv2.FONT_HERSHEY_COMPLEX,0.5,(255,255,0),2)
                                pfreq=Play_Note_Octave(sto,sa[1],1,octave)
                            elif(sat[1]>0.5 and not sat[0]>0.45 and not sat[2]>0.5):
                                cv2.putText(frame,"Нота: Ga/E",(40,130),cv2.FONT_HERSHEY_COMPLEX, 0.5, (255,0, 0), 2)
                                cv2.putText(frame,"Частота: "+str(sa[2]*octave),(40,150),cv2.FONT_HERSHEY_COMPLEX,0.5,(255,255,0),2)
                                pfreq=Play_Note_Octave(sto,sa[2],1,octave)
                            elif(sat[2]>0.45 and not sat[1]>0.45 and not sat[0]>0.4):
                                cv2.putText(frame,"Нота: Ma'/F#",(40,130),cv2.FONT_HERSHEY_COMPLEX, 0.5, (255,0, 0), 2)
                                cv2.putText(frame,"Частота: "+str(sa[3]*octave),(40,150),cv2.FONT_HERSHEY_COMPLEX,0.5,(255,255,0),2)
                                pfreq=Play_Note_Octave(sto,sa[3],1,octave)
                            elif(sat[0]>0.45 and sat[1]>0.5 and not sat[2]>0.5):
                                cv2.putText(frame,"Нота: Pa/G",(40,130),cv2.FONT_HERSHEY_COMPLEX, 0.5, (255,0, 0), 2)
                                cv2.putText(frame,"Частота: "+str(sa[4]*octave),(40,150),cv2.FONT_HERSHEY_COMPLEX,0.5,(255,255,0),2)
                                pfreq=Play_Note_Octave(sto,sa[4],1,octave)
                            elif(sat[1]>0.3 and sat[2]>0.4 and not sat[0]>0.4):
                                cv2.putText(frame,"Нота: Dha/A",(40,130),cv2.FONT_HERSHEY_COMPLEX, 0.5, (255,0, 0), 2)
                                cv2.putText(frame,"Частота: "+str(sa[5]*octave),(40,150),cv2.FONT_HERSHEY_COMPLEX,0.5,(255,255,0),2)
                                pfreq=Play_Note_Octave(sto,sa[5],1,octave)
                            elif(sat[1]>0.4 and sat[2]>0.4 and sat[0]>0.385):
                                cv2.putText(frame,"Нота: Ni/B",(40,130),cv2.FONT_HERSHEY_COMPLEX, 0.5, (255,0, 0), 2)
                                cv2.putText(frame,"Частота: "+str(sa[6]*octave),(40,150),cv2.FONT_HERSHEY_COMPLEX,0.5,(255,255,0),2)
                                pfreq=Play_Note_Octave(sto,sa[6],1,octave)
                            elif(sat[1]<0.4 and sat[2]<0.4 and sat[0]<0.4 and sat[4]>0.45):
                                if octave==1/2 :
                                    cv2.putText(frame,"Нота: Sa'/C",(40,130),cv2.FONT_HERSHEY_COMPLEX, 0.5, (255,0, 0), 2)
                                    cv2.putText(frame,"Частота: "+str(sa[0]*(octave*2)),(40,150),cv2.FONT_HERSHEY_COMPLEX,0.5,(255,255,0),2)
                                    cv2.putText(frame,"Octave: 4",(40,170),cv2.FONT_HERSHEY_COMPLEX, 0.5, (255,0,0), 2)
                                    pfreq=Play_Note_Octave(sto,sa[0],1,(octave*2))
                                else:
                                    cv2.putText(frame,"Нота: Sa'/C",(40,130),cv2.FONT_HERSHEY_COMPLEX, 0.5, (255,0, 0), 2)
                                    cv2.putText(frame,"Частота: "+str(sa[0]*(octave)),(40,150),cv2.FONT_HERSHEY_COMPLEX,0.5,(255,255,0),2)
